...
namespace Template.Persistence.Database
{
    public class PostgreSqlDatabaseManager : IDatabaseManager
    {
        ...
        public PostgreSqlDatabaseManager(IConfigManager configManager)
        {
            _configManager = configManager;

        }

        public PostgreSqlDatabaseManager(string connectionName, IConfigManager configManager)
        {
        ...
        public IDbTransaction BeginTransaction(System.Data.IsolationLevel isolationLevel)
        {
            ...
            _transaction = connection.BeginTransaction(isolationLevel) as NpgsqlTransaction;
            return _transaction;
        }
        ...
        public int Insert(string sql, object param)
        {
            ...
            if (transaction != null)
            {
                ...
                identity = connection.ExecuteScalar<int>(sql, param, transaction);
            }
            else
            {
                try
                {
                    connection.Open();
                    identity = connection.ExecuteScalar<int>(sql, param);
                }
                finally
                {
        ...
        public void BulkInsert(string sql, List<object> param)
        {
            ...
            if (transaction != null)
            {
                ...
                {
                    connection.ExecuteScalar<int>(sql, item, transaction);
                }
            }
            else
            {
                try
                {
                    ...
                    foreach (var item in param)
                    {
                        connection.ExecuteScalar<int>(sql, item);
                    }
                }
                finally
                {
